...
function show_uploads_data_section(){
	...
	setInterval(update_uploads_table, 3000); // update the table every second
...
/*****************************************************************************************************************/
var sent_req = false;
var received_resp = false;
function update_uploads_table(){
	if(!$("#hc_notification_drop_down").hasClass("open")){
		return;
	}
	//console.log("update_uploads_table called");
	if(sent_req && received_resp){
		received_resp =false;
		sent_req =false;
	}
	if(sent_req && !received_resp){
	...
	hc_utils_send_request_to_server("post_uploads","POST","",{},function(data){
...
function show_table_uploads(data){
	...
	data.forEach(function(file_rec){
		...
		if(/*file_rec.file_status=="Pending" &&*/ is_record_in_queue(file_rec)){// lets see if we have this notification already queued
			...
			var op_percentage = Math.round(file_rec.file_size ==0 ?0: file_rec.op_percentage * 100 / file_rec.file_size);
