	...
	function hc_mycloudcontent_go_to_dir(dest_dir){
		...
		if(dest_dir == "/Home Cloud") dest_dir = "";
	...
	function get_badges_of_file(filename,filepath){
		...
		for(var i=0;i<file_sys_view.file_sys_badge.length;i++){
			...
			if(element_clean_fn == get_clean_path(filename)){
				...
				res.is_in_trash = element.is_in_trash;
	...
	function hc_mycloudcontent_manipulate_filesystem_view(wanted_dir){
		...
		var res = {"data":[]};
		if( JSON.stringify(file_sys_view.file_sys_repo) === []){
			return res;
		}
		if(wanted_dir == ""){
			//return the drives only
			for(i=0;i<file_sys_view.mounted_drives.length;i++){
				res.data.push({"file_size":file_sys_view.mounted_drives[i].size,"file_name":file_sys_view.mounted_drives[i].mounted_dir,"file_type":'hdd-o',"file_modified_date":"","file_ext":"d","deluxe_badges":""});
			}
			return res;
		}
		file_sys_view.file_sys_repo.forEach(function(curr,index,element){
			...
			if(curr.file_name.indexOf(wanted_dir) == 0){
				...
				if(wanted_file_name != "" &&get_clean_path( wanted_dir+"/"+wanted_file_name) == get_clean_path(curr.file_name)){
					...
					if(manipulated_curr.deluxe_badges.is_in_trash){
						return; // dont show deleted files or folders
					}
	...
	/*****************************************************************************************************************/
	function parse_ls_line(ls_output){
		if(ls_output == undefined || ls_output == null || ls_output == ""){
			return [];
		}
		...
		ls_output.forEach(function(elem){
			if(elem == "")return;
			var elem_arr = elem.split(" ").filter(function(n){ return n != "" && n!= undefined && n != null; }); 
			var file_name = elem_arr.splice(4).join(" ");
			res.push({"file_type":elem_arr[0][0], "file_size":elem_arr[2],"file_modified_date":elem_arr[3],"file_name":file_name})
		});
		return res;
	
	}
	/*****************************************************************************************************************/
	function hc_mycloudcontent_servercall_getCloudContentAndUpdateTable(file_path){
		...
		hc_utils_send_request_to_server("getdeluxecontent","POST","",{},function(data){
			...
			hc_utils_send_request_to_server("getsharedlinks","GET","",{},function(data){
				...
				hc_utils_send_request_to_server("GetFolderContents?folderpath="+file_path,"GET","",{},function(data){
					file_sys_view.file_sys_repo.push(...parse_ls_line(data.data));
	...
	function hc_mycloudcontent_handle_doubleclick(file_name){
		...
		for(var i =0;i<file_sys_view.file_sys_repo.length;i++){
			var item = file_sys_view.file_sys_repo[i];
			var looking_for_full_path = "/" + file_sys_view.curr_dir+ "/"+file_name.trim();
			var is_storage_filename = is_storage(file_name);
			if(get_clean_path(item.file_name) == get_clean_path(looking_for_full_path) || is_storage_filename){
				if(item.file_type == 'd' || is_storage_filename){
					console.log("it is equal and the clicked item is a folder");
					file_sys_view.curr_dir = looking_for_full_path + '/';
					var manipulated_data = hc_mycloudcontent_manipulate_filesystem_view(file_sys_view.curr_dir);
					hc_mycloudcontent_set_inactive_directory(file_sys_view.curr_dir);
					if(manipulated_data.data.length == 0){
						//we don't have the files of this path....need to import them.
						hc_mycloudcontent_servercall_getCloudContentAndUpdateTable(looking_for_full_path);
						return;
					}
					file_sys_view.curr_dir = looking_for_full_path;
					hc_mycloudcontent_updateFilesTable(manipulated_data);
	...
	function hc_delete_file_or_folder(){
		var reqbody = {};
		reqbody.operation_type = "move_to_trash";
		
		send_deluxe_operation(reqbody,function(res){
			console.log("received a response" + JSON.stringify(res));
			update_local_badge(filename,filepath,"move_to_trash");
		});
	}
	/*****************************************************************************************************************/
	...
	/*****************************************************************************************************************/
	function update_local_badge(filename,filepath,operation_type){
		for(var i=0;i<file_sys_view.file_sys_badge.length;i++){
			var element = file_sys_view.file_sys_badge[i];
			if(element.file_name.trim() == filename.trim() && element.file_path == filepath){
				if(operation_type == "add_to_favorites"){
					file_sys_view.file_sys_badge[i].is_favorite=file_sys_view.file_sys_badge[i].is_favorite==true?false:true;
					return;
				}
				if(operation_type == "move_to_trash"){
					file_sys_view.file_sys_badge[i].is_in_trash=true;
					return;
				}
			}
		...
		if(operation_type == "move_to_trash"){
			file_sys_view.file_sys_badge.push({"file_name":filename,"file_path":filepath,"is_in_trash":true});
		}
	}
	/*****************************************************************************************************************/
	...
	function hc_add_to_favorites_file_or_folder(){
		...
		send_deluxe_operation(reqbody,function(res){
			...
			if(res.status == 200){
				...
				//update the star
				update_local_badge(filename,filepath,"add_to_favorites");
				hc_mycloudcontent_go_to_dir(file_sys_view.curr_dir);
